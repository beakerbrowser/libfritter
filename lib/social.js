...
class LibFritterSocialAPI {
  ...
  async setAvatar (archive, imgData, extension) {
    archive = this.db._archives[toUrl(archive)]
  ...
  async follow (archive, target, name) {
    ...
    // index the target
    await this.db.addSource(target)
  }
  ...
  async unfollow (archive, target) {
    ...
    // unindex the target
    await this.db.removeSource(target)
  }
  ...
  async isFollowing (archiveA, archiveB) {
    var archiveAUrl = toUrl(archiveA)
    var archiveBUrl = toUrl(archiveB)
